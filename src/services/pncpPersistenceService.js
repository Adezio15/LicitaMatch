export function pncpRepository(database) {
  return {
    async save(item) {
      const { rows } = await database.query(`INSERT INTO licitacoes_pncp (codigo_externo, objeto, data_abertura, unidade_gestora, modalidade, origem)
        VALUES ($1,$2,$3,$4,$5,$6)
        ON CONFLICT (codigo_externo) DO NOTHING
        RETURNING id, codigo_externo, objeto, data_abertura, unidade_gestora, modalidade, origem`,
      [item.id, item.objeto, new Date(item.dataAbertura).toISOString(), item.unidadeGestora, item.modalidade || 'N/D', 'pncp']);
      return rows[0] || null;
    },
...
export async function persistPncpItems(database, items = []) {
